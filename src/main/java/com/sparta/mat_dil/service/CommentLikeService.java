...
public class CommentLikeService {
    ...
    //좋아요한 댓글 리스트
    public Page<CommentResponseDto> getLikeCommentList(int page, User user) {
        //댓글 등록 순으로 정렬
        ...
        return new PageImpl<>(commentLikeRepository.getAllLikeComment(user.getAccountId(), pageable).stream().map(CommentResponseDto::new).collect(Collectors.toList()));
    }
}
