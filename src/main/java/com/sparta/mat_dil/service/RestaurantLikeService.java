...
public class RestaurantLikeService {
    ...
    public Page<RestaurantResponseDto> getLikeRestaurantList(int page, User user) {
        //음식점 등록 순으로 정렬
        ...
        return new PageImpl<>(restaurantLikeRepository.getAllLikeRestaurant(user.getAccountId(), pageable).stream().map(RestaurantResponseDto::new).collect(Collectors.toList()));
